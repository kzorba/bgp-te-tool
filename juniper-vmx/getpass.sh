...
for vmx in $list; do
  ...
  if [ ! -z "$descr" ]; then
    ...
    fpcmem=$(ssh -o StrictHostKeyChecking=no -o ConnectTimeout=1 -i ../napalm-ssh-keys/lab_id_rsa user@$ip $CLI show chassis fpc 0 2>/dev/null | grep Online | awk '{print $9}')
    fpcmem="${fpcmem:-0}"
